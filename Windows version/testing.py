...
if config.sys_lan == "en-US":
    text_surface = font.render(
        languages.lan_en["en"]["ver"] + version,
        True,
        (240, 240, 240)
    )
else:
    text_surface = font.render(
        languages.lan_ru["ru"]["ver"] + version,
        True,
        (240, 240, 240)
    )
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                # Выбор языка
                elif 220 <= x <= 252 and 543 <= y <= 575 and settings_s:
                    settings_l = True
                    settings_s = False
    ...
    if config.sys_lan == "en-US":
        title_surface = font.render(
            languages.lan_en["en"]["title"] + i_name,
            True,
            (240, 240, 240)
        )
    else:
        title_surface = font.render(
            languages.lan_ru["ru"]["title"] + i_name,
            True,
            (240, 240, 240)
        )
    ...
    if not enter and text == "":
        if config.sys_lan == "en-US":
            placeholder_surface = font.render(
                languages.lan_en["en"]["placeh"],
                True,
                (0, 0, 0)
            )
        else:
            placeholder_surface = font.render(
                languages.lan_ru["ru"]["placeh"],
                True,
                (0, 0, 0)
            )
    ...
    if panel:
        ...
        if config.sys_lan == "en-US":
            settings_surface = font.render(
                languages.lan_en["en"]["sett"],
                True,
                (240, 240, 240)
            )
        else:
            settings_surface = font.render(
                languages.lan_ru["ru"]["sett"],
                True,
                (240, 240, 240)
            )
    ...
    if settings_s and settings_l == False:
        ...
        if config.sys_lan == "en-US":
            language_surface = font.render(
                languages.lan_en["en"]["lang"],
                True,
                (240, 240, 240)
            )
        else:
            language_surface = font.render(
                languages.lan_ru["ru"]["lang"],
                True,
                (240, 240, 240)
            )
